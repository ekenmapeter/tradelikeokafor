        ...
        <form action="{{ route('admin.subscriptions.store') }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div x-data="{ 
                    ...
                    <label for="user_search" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Select User</label>
                                ...
                                <template x-for="user in filteredUsers" :key="user.id">
                                    <li @click="selectUser(user)" 
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span x-text="'(' + user.email + ')'" class="ml-2 text-xs text-gray-500 group-hover:text-indigo-200 truncate"></span>
                                        </div>
                                    </li>
                                </template>
                                <li x-show="filteredUsers.length === 0" class="text-gray-500 dark:text-gray-400 py-2 pl-3">
                                    No users found.
                                </li>
                            </ul>
                        </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', date('Y-m-d')) }}" required 
                     ...
                     @error('start_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Subscription will have no expiration date (Lifetime Access).</p>
                </div>
            </div>
