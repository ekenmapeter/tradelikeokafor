...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.forex-raw.index') }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                View Pending Raw Articles
            </a>
            <a href="{{ route('admin.forex-drafts.pipeline') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition">
    ...
    <form id="bulkForm" action="{{ route('admin.forex-drafts.bulk-approve') }}" method="POST">
        ...
        <div class="flex items-center gap-3 mb-4">
            <label class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                <input type="checkbox" id="selectAll" class="mr-2 rounded border-gray-300 text-green-600 focus:ring-green-500"> Select All
            </label>
            <button type="submit" id="bulkApproveBtn" style="display:none" class="items-center px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-medium hover:bg-green-700" onclick="return confirm('Approve all selected?')">
                ✓ Bulk Approve
            </button>
        </div>
        @endif
        ...
        <div class="space-y-3">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md transition-shadow">
                <div class="flex items-start gap-4">
                    ...
                    <input type="checkbox" name="draft_ids[]" value="{{ $draft->id }}" class="draft-checkbox mt-1 rounded border-gray-300 text-green-600 focus:ring-green-500">
                    @endif
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            @if($draft->rawArticle)
                            <span class="text-xs text-gray-400">from {{ $draft->rawArticle->source_name }}</span>
                            @endif
                            <span class="text-xs text-gray-400">{{ $draft->created_at->diffForHumans() }}</span>
                        </div>
                        <a href="{{ route('admin.forex-drafts.show', $draft) }}" class="text-lg font-semibold text-gray-900 dark:text-white hover:text-green-600 transition line-clamp-1">{{ $draft->ai_title }}</a>
                        <p class="text-sm text-gray-500 mt-1 line-clamp-2">{{ $draft->ai_excerpt ?: Str::limit(strip_tags($draft->ai_content), 150) }}</p>
                        @if($draft->parsed_tags)
                        <div class="flex flex-wrap gap-1 mt-2">
                            @foreach(array_slice($draft->parsed_tags, 0, 4) as $tag)
                            <span class="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded text-xs">{{ $tag }}</span>
                            @endforeach
                        </div>
                        @endif
                        @if($draft->status === 'rejected' && $draft->reject_reason)
                        <div class="mt-2 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg text-xs text-red-600">
                            <strong>Reason:</strong> {{ $draft->reject_reason }}
                        </div>
                        @endif
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <a href="{{ route('admin.forex-drafts.show', $draft) }}" class="p-2 text-gray-400 hover:text-blue-600 transition" title="Preview">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        @if($draft->status === 'draft')
                        <a href="{{ route('admin.forex-drafts.edit', $draft) }}" class="p-2 text-gray-400 hover:text-yellow-600 transition" title="Edit">
                        ...
                        <form action="{{ route('admin.forex-drafts.approve', $draft) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="p-2 text-gray-400 hover:text-green-600 transition" title="Approve" onclick="return confirm('Approve and publish?')">
            ...
            <div class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-400 mt-1">Go to <a href="{{ route('admin.forex-drafts.pipeline') }}" class="text-green-600 hover:underline">Pipeline</a> to generate new drafts.</p>
            </div>
            @endforelse
        </div>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(sa) sa.addEventListener('change', function(){ cbs.forEach(c => c.checked = this.checked); toggle(); });
    cbs.forEach(c => c.addEventListener('change', toggle));
    function toggle(){ if(btn) btn.style.display = document.querySelectorAll('.draft-checkbox:checked').length > 0 ? 'inline-flex' : 'none'; }
});
</script>
@endsection
